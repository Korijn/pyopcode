...
#define __OPC_TREEBUILDERS_H__
	...
	class OPCODE_API AABBTreeOfVerticesBuilder : public AABBTreeBuilder
	{
		...
		_override(AABBTreeBuilder)	bool			ComputeGlobalBox(const udword* primitives, udword nb_prims, IceMaths::AABB& global_box)	const;
		_override(AABBTreeBuilder)	float			GetSplittingValue(udword index, udword axis)									const;
		_override(AABBTreeBuilder)	float			GetSplittingValue(const udword* primitives, udword nb_prims, const IceMaths::AABB& global_box, udword axis)	const;
